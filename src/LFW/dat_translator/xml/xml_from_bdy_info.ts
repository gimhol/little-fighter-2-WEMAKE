...
export function xml_from_bdy_info(xml: IXML, b: IBdyInfo, tag: string = "bdy"): IXMLElement {
  ...
  el.set_attr("id", b.id);
  el.set_attr("name", b.name);
  el.set_attr("ref", b.ref);
  el.set_attr("kind", b.kind as number);
  el.set_attr("hit_flag", b.hit_flag as number);
  b.actions?.map(v => xml_from_colli_action(xml, v, "action")).forEach(v => {
    el.insert(v);
  })
  el.set_arr_attr_soft("qube", [b.x, b.y, b.w, b.h, b.z, b.l]);
  el.set_attr("test", b.test);
  el.set_attr("code", b.code);
  return el;
}
