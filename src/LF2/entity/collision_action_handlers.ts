import type { ICollision } from "../base";
import type { IActionHandler } from "../base/IActionHandler";
import { ActionType as AT } from "../defines/ActionType";
...
export const collision_action_handlers: IActionHandler = {
  ...
  [AT.A_NextFrame]: (a, c) => c.attacker.next_frame = c.attacker.get_next_frame(a.data)?.which ?? c.attacker.next_frame,
  a_set_prop: (a, c) => (c.attacker as any)[a.prop_name] = a.prop_value,
  [AT.A_BrokenDefend]: (a, c) => collision_action_handlers[AT.A_NextFrame](a, c),
  [AT.A_Defend]: (a, c) => collision_action_handlers[AT.A_NextFrame](a, c),

  [AT.V_Sound]: (a, c) => c.victim.play_sound(a.data.path, a.data.pos),
  [AT.V_NextFrame]: (a, c) => c.victim.next_frame = c.victim.get_next_frame(a.data)?.which ?? c.victim.next_frame,
  v_set_prop: (a, c) => (c.victim as any)[a.prop_name] = a.prop_value,
  [AT.V_BrokenDefend]: (a, c) => collision_action_handlers[AT.V_NextFrame](a, c),
  [AT.V_Defend]: (a, c) => collision_action_handlers[AT.V_NextFrame](a, c),

  [AT.A_REBOUND_VX]: function (action: IAction_ReboundVX, collision: ICollision) {
    const { attacker } = collision;
    attacker.set_velocity_x(-attacker.velocity.x);
  },
  [AT.V_REBOUND_VX]: function (action: IAction_ReboundVX, collision: ICollision) {
    const { victim } = collision;
    victim.set_velocity_x(-victim.velocity.x);
  },
  [AT.V_TURN_FACE]: function (action: IAction_TurnFace, collision: ICollision) {
    const { victim } = collision;
    victim.facing = turn_face(victim.facing);
  },
  [AT.V_TURN_TEAM]: function (action: IAction_TurnTeam, collision: ICollision) {
    const { victim, attacker } = collision;
    victim.team = attacker.team;
  },
  [AT.FUSION]: function (action: IAction_Fusion, collision: ICollision) {
  ...
  [AT.BROADCAST]: (action: IAction_Broadcast, { lf2 }: ICollision) => lf2.broadcast(action.data)
